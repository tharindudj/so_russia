...
adminCommands( cmd, pickingType ) {
	...
	case "kill":
		...
		{		
			...
			iPrintln( "^1SO^2[Admin]:^7 " + player.name + " ^7killed." );
		}
		break;
	...
	case "bounce":
		...
		{		
			for( i = 0; i < 2; i++ )
			{
				...
				iPrintln( "^1SO^2[^1SO^2[Admin]]: ^7Bounced " + player.name + "^7." );
			}
		}
		break;
	...
	case "tphere":
		...
		if(isDefined(toport) && isDefined(caller) ) 
		{
			...
			iPrintln( "^1SO^2[Admin]:^1 " + toport.name + " ^7was teleported to ^1" + caller.name + "^7." );
		}
		break;
