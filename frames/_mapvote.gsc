...
init()
{
	if(getentarray("player", "classname").size < 6)
		setDvar( "sv_maprotation", "gametype sr map mp_marketcenter gametype sd map mp_backlot gametype sd map mp_citystreets gametype sd map mp_island_town gametype sr map mp_berry_village gametype sd map mp_naout gametype sr map mp_slick gametype sd map mp_crash gametype sd map mp_crossfire gametype sd map mp_strike gametype sd map mp_backlot_night gametype sd map mp_bubba gametype sr map mp_nuketown" );
	...
	//text
	hud[3] = addTextHud( level, 0, level.windowheight/-2-8, 1, "center", "bottom", "center", "middle", 2.2, 101 );
	hud[3] setText("^2Mapvote");
...
PlayerVote() {
	...
	self.mapvote_selection.color = (0.07, 0.09, 0.2);
...
getMapNameString( mapName )  {
	switch( toLower( mapName ) ) {
        ...
        case "mp_island_town":
			mapName = "Island Town";
			break;
        case "mp_backlot_night":
			mapName = "Backlot Night";
			break;
