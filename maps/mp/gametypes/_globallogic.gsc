...
init()
{
	...
	precacheShader("first_blood");

	precacheShader("profile");

	precacheShader("so_logo");
	precacheShader("so_starting_in");
	...
	level.fx_smoke_mp = loadFX ("props/hunnia_smoke_mp");
	level.fx_smoke_hallway = loadFX ("smoke/hallway_smoke_light");

	level.fx_so_run_attack = loadfx("so/so_run_attack");
	level.fx_so_run_defence = loadfx("so/so_run_defence"); 	
	level.fx_dust1 = loadfx("so/dust1");	
	level.fx_dust2 = loadfx("so/dust2");
...
matchStartTimer()
{
	...
	//if(getDvar("g_gametype") == "war")setDvar( "g_gametype", "sd" ); // Killhouse TDM only	
}
...
endGame(winner,endReasonText)
{
	...
	//duffman\_endfights::startEndFight();
	//duffman\_endfade::init();
...
Callback_StartGameType()
{
	...
	if(!isDefined(game["gamestarted"]))
	{
		...
		game["colors"]["blue"] = (0.35, 0.7, 0.8); //5ab3cc
	...
	//thread duffman\afkmonitor::init();
...
Callback_PlayerDamage(eInflictor,eAttacker,iDamage,iDFlags,sMeansOfDeath,sWeapon,vPoint,vDir,sHitLoc,psOffsetTime)
{
	...
	if( sWeapon=="m40a3_mp" || sWeapon=="remington700_mp"  ){
		iDamage += 12; // Damage + 20
	}
...
Callback_PlayerKilled(eInflictor,attacker,iDamage,sMeansOfDeath,sWeapon,vDir,sHitLoc,psOffsetTime,deathAnimDuration)
{
	...
	if(!isDefined(self.isKnifing))
	{
		...
		{
			attacker thread crazy\splash::killedPlayer( self, sWeapon, sMeansOfDeath );
			//attacker thread crazy\fieldorders::onKill(self);
		}
	}
	...
	if(!level.rdyup && !isDefined(attacker.isKnifing))
	{
		...
		if(isDefined(attacker.pers)&&!isDefined(self.switching_teams))
		{
			...
			self maps\mp\gametypes\_persistence::statGet( "deaths" );
		}
	}
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					attacker maps\mp\gametypes\_persistence::statSet( "kills", attacker.kills );
					attacker maps\mp\gametypes\_persistence::statGet( "kills" );
	...
	//if ( isDefined( attacker ) && isPlayer( attacker ) && attacker != self && (!level.teambased || level.gametype != "sd" || attacker.pers["team"] != self.pers["team"]) )
		//self thread crazy\_missions::playerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, sHitLoc );
	...
	self.cur_kill_streak = 0;
	if(isDefined(attacker)) attacker.cur_kill_streak++;		
	attacker maps\mp\gametypes\_persistence::statSet( "kill_streak", attacker.cur_kill_streak );
	attacker maps\mp\gametypes\_persistence::statGet( "kill_streak" );
...
Joinknifegame(dowait)
{
	...
	self thread lmsg("^1[Melee] ^5To Play Knife");
...
AutoTeamsBalancer() {
	...
	while(1) {
		...
		if (isDefined(game["state"]) && game["state"] == "playing") {
			...
			if (offset < 2)
				continue;
			iPrintln("^1Teams will be balanced in 5 sec...");
			wait 5;
			if (isDefined(game["state"]) && game["state"] == "playing") {
				team["axis"] = getTeamPlayers("axis");
				team["allies"] = getTeamPlayers("allies");
				if(team["axis"].size == team["allies"].size) {
					iPrintln("^1AutoBalance aborted: teams are already balanced!");
					continue;
				}
				if(team["axis"].size < team["allies"].size) {
					changeteam = "axis";
