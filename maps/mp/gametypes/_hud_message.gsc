/*
  Copyright (c) 2009-2017 Andreas G�ransson <andreas.goransson@gmail.com>
...
initNotifyMessage()
{
	...
	self.notifyIconX = createIcon( "white", iconSize, iconSize );
	self.notifyIconX setParent( self.notifyIcon );
	self.notifyIconX setPoint( point, relativePoint, 0, -65 );
	self.notifyIconX.hideWhenInMenu = true;
	self.notifyIconX.archived = false;
	self.notifyIconX.alpha = 0;
...
showNotifyMessage( notifyData )
{
	...
	if ( isDefined( notifyData.iconName ) )
	{
		...
		self.notifyIconX setParent( anchorElem );
		self.notifyIconX setShader( "so_logo", 280, 140 );
		self.notifyIconX.alpha = 0;
		self.notifyIconX fadeOverTime( 1 );
		self.notifyIconX.alpha = 1;

		waitRequireVisibility( duration-2 );

		self.notifyIconX fadeOverTime( 0.75 );
		self.notifyIconX.alpha = 0;
	}
	else
		waitRequireVisibility( duration );
...
teamOutcomeNotify( winner, isRound, endReasonText, delay )
{
	...
	scoreDisplay1 = createIcon( "line_horizontal", 700, 50 );
	scoreDisplay1 setParent( outcomeTitle );
	scoreDisplay1 setPoint( "TOP", "BOTTOM", 0, 70 );
	scoreDisplay1.hideWhenInMenu = false;
	scoreDisplay1.color = (0, 0 ,0);
	scoreDisplay1.archived = false;
	scoreDisplay1.alpha = 1;
	scoreDisplay1 fadeOverTime( 0.5 );
	
	scoreDisplay2 = createIcon( "line_horizontal", 700, 50 );
	scoreDisplay2 setParent( outcomeTitle );
	scoreDisplay2 setPoint( "TOP", "BOTTOM", 0, 70 );
	scoreDisplay2.hideWhenInMenu = false;
	scoreDisplay2.color = (0, 0 ,0);
	scoreDisplay2.archived = false;
	scoreDisplay2.alpha = 1;
	scoreDisplay2 fadeOverTime( 0.5 );
	...
	hudElems[hudElems.size] = addTextHud( level, 320, 80+addY, 1, "center", "middle", 2 );
	hudElems[hudElems.size-1] setText( "^3YOUR PLAYING SUMMARY" );
	
	info[0]["stat"] = "score";
	info[0]["hud"] = addIconHud( level, 290, 110+addY, 0, "ui_host", 40, 40 );
	info[0]["hud"].sort = -1;
	info[1]["stat"] = "kills";
	info[1]["hud"] = addIconHud( level, 290, 110+addY, 0, "killiconsuicide", 40, 40 ); // @FIXME
	info[1]["hud"].sort = -1;
	info[2]["stat"] = "knifes";
	info[2]["hud"] = addIconHud( level, 290, 110+addY, 0, "killiconmelee", 40, 40 );
	info[2]["hud"].sort = -1;
	info[3]["stat"] = "headshots";
	info[3]["hud"] = addIconHud( level, 290, 110+addY, 0, "killiconheadshot", 40, 40 );
	info[3]["hud"].sort = -1;
	info[4]["stat"] = "time";
	info[4]["hud"] = addIconHud( level, 290, 110+addY, 0, "hudstopwatch", 40, 40 ); // @FIXME
	info[4]["hud"].sort = -1;
	...
	outcomeText.sort = 900;
	
	outcomeTextX = createFontString( font, 2 );
	outcomeTextX setParent( outcomeText );
	outcomeTextX setPoint( "TOP", "BOTTOM", 0, 60 );
	outcomeTextX.glowAlpha = 1;
	outcomeTextX.hideWhenInMenu = false;
	outcomeTextX.archived = false;
	outcomeTextX.sort = 902;
	
	if ( winner == "halftime" )
	{
		outcomeTitle.glowColor = (0.1, 0.0, 1);
		outcomeTitle setText( game["strings"]["halftime"] );
		outcomeTitle.color = (0, 0, 1);
	}
	...
	else if ( winner == "roundend" )
	{
		...
		outcomeTitle.color = (0, 1, 0);
	}
	else if ( winner == "overtime" )
	{
		outcomeTitle.glowColor = (0.1, 1, 0.1);
	...
	{
		outcomeTitle.glowColor = (1, 0,0.1);
		if ( isRound )
	...
	rightIcon setPoint( "TOP", "BOTTOM", 180, spacing-24 );
	...
	leftScore setPoint( "TOP", "BOTTOM", 100, spacing-120 );
	...
	rightScore setPoint( "TOP", "BOTTOM", -100, spacing-120 );
